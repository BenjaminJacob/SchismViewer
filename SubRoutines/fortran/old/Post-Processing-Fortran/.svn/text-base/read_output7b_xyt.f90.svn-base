...
!									
!       pgf90 -O2 -mcmodel=medium -Mbounds -Bstatic -o read_output7b_xyt read_output7b_xyt.f90 ~/SELFE/svn/trunk/src/Utility/UtilLib/compute_zcor.f90 ~/SELFE/svn/trunk/src/Utility/UtilLib/pt_in_poly.f90
!****************************************************************************************
!
      program read_out
      parameter(nbyte=4)
      ...
      open(10,file='read_output7b_xyt.in',status='old')
      read(10,'(a30)')file63
...
!     Horizontal grid
      ...
      irec=irec+2
      allocate(x(np),y(np),dp(np),kbp00(np),kfp(np),i34(ne),elnode(4,ne), &
...
!       Interpolate in time
        ...
        if(i23d==2) then
          ...
          write(18,'(e16.8,2(1x,f12.3))')t00(i)/86400+start_corie,out4(1,1:ivs)
        else !3D
          if(iep(i)==0) then !no parents
            out4(:,1:ivs)=rjunk
          else
          ...
          do k=nvrt,kbpl,-1
            ...
            write(18,'(i6,4(1x,f12.3))')i,out4(k,1),ztmp(k)-ztmp(nvrt),ztmp(k),t00(i)/86400+start_corie 
            if(ivs==2) write(19,'(i6,4(1x,f12.3))')i,out4(k,2),ztmp(k)-ztmp(nvrt),ztmp(k),t00(i)/86400+start_corie
          enddo !k
        endif
      enddo !i=1,nxy
