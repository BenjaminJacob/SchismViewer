...
!
      ...
      allocatable :: idry(:),outs(:,:,:),residual(:,:)
      ...
      print*, 'Input z-coord. (<=0 in general):'
      read(*,*) z00
...
!     Horizontal grid
     ...
     &eta2(np),xctr(ne),yctr(ne),dpe(ne),kbe(ne),ztmp(nvrt,np),residual(np,2),stat=istat)
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &sigma,ztmp(:,i),idry(i),kbpl)
            ...
            if(idry(i)==1) then
              tmp=-huge(1.0)
              residual(i,:)=residual(i,:)+tmp/ndays/nrec
            else !wet
...
!'
                ...
                do m=1,ivs
                  ...
                  residual(i,m)=residual(i,m)+tmp/ndays/nrec
                enddo !m
              endif
            endif !idry
          enddo !i=1,np
