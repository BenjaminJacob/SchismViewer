...
! ifort -assume byterecl -O2 -Bstatic -o read_output7_xyz read_output7_xyz.f90 UtilLib/compute_zcor.f90 UtilLib/pt_in_poly.f90
!****************************************************************************************
!
      program read_out
...
!      dimension ivout(100,1000),rvout(100,1000)
      ...
      character(len=500) :: runDir,basedir,fname
      character(len=50) :: binary_file,bp_file

      CALL get_opts_TS(fill_in,runDir,basedir,binary_file,istack1,istack2,bp_file,ics,ifs) 	
      bp_file=adjustl(bp_file)
      leng=len_trim(bp_file)
      fname=bp_file(1:leng)
...
!     Read binary header
      ...
      it_char=adjustl(it_char)
      leng=len_trim(it_char)
      fname = adjustl(trim(basedir))//'/'//it_char(1:leng)//'_'//binary_file
...
!     Read in vgrid.in 
      ...
      call get_vgrid(trim(runDir(1:leng))//'vgrid.in',np,nvrt,ivcor,kz,h_s,h_c,theta_b,theta_f,ztot,sigma,sigma_lcl,kbp)
