function [h,ts,data] = eb_readTimeStep2(h,n,maskIdx)
%[h,ts,data] = eb_readTimeStep2(h,n,[maskIdx])
% read time step(s) from the elcirc binary files, observe reading maskIdx
% h -header structure as returned by readHeader
% n - array of timestep numbers 
% maskIdx - mask of timestep values to read as index ([2 202:1000 ])
% mask reading is most efficeint for contigues slabs of data
%
% sergey frolov march 8, 2004
% modified sf November 17, 2004
...
elseif nargin ==3
    data=zeros([length(maskIdx) h.flagSv length(n)]);
    for i=1:length(n)
        [ts{i},d(:,:,i)]=readTsMask(h,n(i),maskIdx);
    end
else
    error('funny number of arguments')
end
...
function [ts,data]=readTsMask(h,n,maskIdx)
    %cheack that the mask is valid
    if ~(min(maskIdx <=0)&max(maskIdx>h.stepSize))
        error('mask specified is invalid with respect to the ts length')
    end
    startPos    = h.dataStartPos + (h.stepSize)*(n-1) + min(maskIdx);
    recLength   = max(maskIdx) - min(maskIdx) + 1;
...
%     ts          = computeFlagSurf(h,ts);
	...
	data    = fread(h.fid, [h.flagSv recLength], 'float32')';            
    data    = data(maskIdx-min(maskIdx));
